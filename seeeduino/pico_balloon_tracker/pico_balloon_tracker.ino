...
static const uint8_t PROGMEM DEVEUI[8] = {0xad, 0x9e, 0x2c, 0x72, 0xd3, 0xaa, 0xfa, 0xb3}; // Helium or TTN

// This EUI must be in little-endian format, so least-significant-byte (lsb)
// first. When copying an EUI from Helium Console or ttnctl output, this means to reverse the bytes.

static const u1_t PROGMEM APPEUI[8] = {0xd5, 0x04, 0xb4, 0xb6, 0x78, 0x44, 0x1d, 0x01}; // helium or ttn
...
void loop()
{
    ...
    if (((voltage > battMin) && gpsFix) || ((voltage > gpsMinVolt) && !gpsFix))
    {
        ...
        if ((!packetQueued && (millis() - last_packet > TX_INTERVAL) && !(LMIC.opmode & (OP_JOINING | OP_TXRXPEND))) || !gpsFix)
        {
            ...
            if (1 || myGPS.getPVT() && (myGPS.getFixType() != 0) && (myGPS.getSIV() > 3))
            {
                gpsFix = true;
...
void checkRegionByLocation()
{
    ...
    Lorawan_Geofence_region_code = _REGCODE_US915; // default region
}
